...
std::string BLEconnection(BLEClient* pClient){
    ...
    std::string heartValue;
    std::string tempValue;
    std::string bloodPressureValue;
    ...
    if(pRemoteService != nullptr){
      ...
      BLERemoteCharacteristic* pHeartCharacteristic = pRemoteService->getCharacteristic(BLEUUID(CHARACTERISTIC_UUID));
      if(pHeartCharacteristic != nullptr){
        heartValue = pHeartCharacteristic->readValue();
    ...
    BLERemoteCharacteristic* pBloodPressureCharacteristic = pRemoteService->getCharacteristic(BLEUUID(BLOOD_PRESSURE_UUID));
    if (pBloodPressureCharacteristic != nullptr) {
        bloodPressureValue = pBloodPressureCharacteristic->readValue();
        Serial.print("BLE, BLOOD PRESSURE: ");
        Serial.println(bloodPressureValue.c_str());
    }
    ...
    // Invio del messaggio
    if((heartValue.length() > 0) && (tempValue.length() > 0) && (bloodPressureValue.length() > 0)){
      message = std::to_string(NODE_ADDRESS) + ":" + position + ":" + heartValue + ":" + tempValue + ":" + bloodPressureValue + ":" + highValues + "\0";
